...
{
    public partial class QHConfirmForm : Form
    {
        private WLDataModelLayer.WLB OriginalRecord { get; set; }

        public static void InitAndShowDialog()
        {
            using (QHConfirmForm qhConfirmForm = new QHConfirmForm())
            {
                qhConfirmForm.Text = string.Format("办理取货（当前网点：{0}-{1}）", WLDataLogicLayer.Setting.NodeCode, WLDataLogicLayer.Setting.NodeName);
                qhConfirmForm.ShowDialog();
            }
        }
        ...
        public QHConfirmForm()
        {
            InitializeComponent();
        }

        private void QHConfirmForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try { }
            catch (Exception ex) { MessageBox.Show(ex.Message, "异常", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }

        private void QHConfirmForm_Shown(object sender, EventArgs e)
        {
            try
            {
                WLDataLogicLayer.WLB.RefreshCentralQHDateList(this.cboDate);
            }
            catch (Exception ex) { MessageBox.Show(ex.Message, "异常", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            try
            {
                WLDataLogicLayer.WLB.CentralQH(this.OriginalRecord);
        ...
        private void btnRefresh_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.cboDate.Enabled && this.cboDate.SelectedIndex >= 0)
                {
                    switch (this.tabControl.SelectedIndex)
                    {
                        case 1: WLDataLogicLayer.WLB.RefreshCentralQHWaitingList(this.listWaiting, Convert.ToDateTime(this.cboDate.SelectedItem)); break;
                        case 2: WLDataLogicLayer.WLB.RefreshCentralQHAlreadyList(this.listAlready, Convert.ToDateTime(this.cboDate.SelectedItem)); break;
                    }
                }
                else throw new ApplicationException("办理日期不存在，无法获取待取货/已取货列表！");
            }
            catch (Exception ex) { MessageBox.Show(ex.Message, "异常", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }
            ...
            try
            {
                switch (e.KeyCode)
                {
                    ...
                    case Keys.Enter:
                        ...
                        {
                            WLDataModelLayer.WLB item = WLDataLogicLayer.WLB.CentralQHRead(this.txtSN.Text);
                            if (item != null)
                                this.Init(item);
                            else
                                throw new ApplicationException(string.Format("不存在等待取货运单'{0}'！", this.txtSN.Text));
                        }
                        break;
                }
            }

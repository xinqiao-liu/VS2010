...
namespace WLDataAccessLayer
{
    public static class Authorize
    {
        ...
        public static bool Insert(SqlCommand cmd, WLDataModelLayer.Authorize item)
        {
        ...
        public static WLDataModelLayer.Authorize Read(SqlCommand cmd, string code)
        {
            ...
            using (SqlDataReader rdr = cmd.ExecuteReader())
            {
                if (rdr.Read())
                {
                    return new WLDataModelLayer.Authorize(
                        rdr["code"].ToString(),
                        rdr["name"].ToString(),
        ...
        public static List<WLDataModelLayer.Authorize> Reads(SqlCommand cmd)
        {
            List<WLDataModelLayer.Authorize> list = new List<WLDataModelLayer.Authorize>();
            ...
            using (SqlDataReader rdr = cmd.ExecuteReader())
            {
                while (rdr.Read())
                {
                    WLDataModelLayer.Authorize item = new WLDataModelLayer.Authorize(
                        rdr["code"].ToString(),
                        rdr["name"].ToString(),
                        rdr["citycode"].ToString(),
                        rdr["cityname"].ToString(),

    ...
    public partial class DetectCPHForm : Form
    {
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //获取指定日期待结账运单车牌号列表
                List<String> items = WLDataLogicLayer.WLB.GetCPHList(this.MinDate, this.MaxDate);
        ...
        private void listInfo_DoubleClick(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    CPHRelatedForm.InitAndShowDialog(this.list.SelectedItems[0].Tag.ToString(), this.MinDate, this.MaxDate);
                }
            }
            catch (Exception ex) { MessageBox.Show(ex.Message, "异常", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }

...
{
    public partial class XCConfirmForm : Form
    {
        private WLDataModelLayer.WLB OriginalRecord { get; set; }

        public static void InitAndShowDialog()
        {
            using (XCConfirmForm xcConfirmForm = new XCConfirmForm())
            {
                xcConfirmForm.Text = string.Format("办理卸车（当前网点：{0}-{1}）", WLDataLogicLayer.Setting.NodeCode, WLDataLogicLayer.Setting.NodeName);
                xcConfirmForm.ShowDialog();
            }
        }
        ...
        public XCConfirmForm()
        {
            InitializeComponent();
        }

        private void XCConfirmForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            try { }
            catch (Exception ex) { MessageBox.Show(ex.Message, "异常", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }

        private void XCConfirmForm_Shown(object sender, EventArgs e)
        {
            try 
            {
                WLDataLogicLayer.WLB.RefreshCentralXCDateList(this.cboDate);
            }
            catch (Exception ex) { MessageBox.Show(ex.Message, "异常", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            try
            {
                WLDataLogicLayer.WLB.CentralXC(this.OriginalRecord);
        ...
        private void btnRefresh_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.cboDate.Enabled && this.cboDate.SelectedIndex >= 0)
                {
                    switch (this.tabControl.SelectedIndex)
                    {
                        case 1: WLDataLogicLayer.WLB.RefreshCentralXCWaitingList(this.listWaiting, Convert.ToDateTime(this.cboDate.SelectedItem)); break;
                        case 2: WLDataLogicLayer.WLB.RefreshCentralXCAlreadyList(this.listAlready, Convert.ToDateTime(this.cboDate.SelectedItem)); break;
                    }
                }
                else throw new ApplicationException("办理日期不存在，无法获取待卸车/已卸车列表！");
            }
            catch (Exception ex) { MessageBox.Show(ex.Message, "异常", MessageBoxButtons.OK, MessageBoxIcon.Error); }
        }
            ...
            try
            {
                switch (e.KeyCode)
                {
                    ...
                    case Keys.Enter:
                        ...
                        {
                            WLDataModelLayer.WLB item = WLDataLogicLayer.WLB.CentralXCRead(this.txtSN.Text);
                            if (item != null)
                                this.Init(item);                                
                            else
                                throw new ApplicationException(string.Format("不存在等待卸车运单'{0}'！", this.txtSN.Text));
                        }
                        break;
                }
            }
